    ...
    public int onStartCommand(final Intent intent, int flags, int startId) {
        ...
        if (isRecording){
            ...
            final Runnable mRecordRunnable = new Runnable() {
                @Override
                public void run() {
                    AudioRecorder.getsInstance().recordAudioFile(RecordService.this);
                    // Stops recording in 10 second period
                    mServiceHandler.postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            AudioRecorder.getsInstance().stopRecordAudioFile(RecordService.this);
                        }
                    }, 10000);
                }
            };
            ...
            AudioRecorder.getsInstance().stopRecordAudioFile(this);
        }
        return START_STICKY;
    }
